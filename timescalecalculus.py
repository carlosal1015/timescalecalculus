import operator
# Added this because in python 3.* they changed the location of the reduce() method to the functools module
...
class timescale:
    def __init__(self, ts, name='none'):
        ...
        for listItem in ts:
            if isinstance(listItem, list):
                if len(listItem) > 2:
                    raise Exception(
                        "Invalid timescale declaration: you cannot have an interval with more than one starting and one ending value.")

                if len(listItem) < 2:
                    raise Exception(
                        "Invalid timescale declaration: an interval must have a starting value and an ending value.")

                if listItem[0] > listItem[1]:
                    raise Exception(
                        "Invalid timescale declaration: you cannot have an interval in which the ending value is smaller than the starting value.")

                if listItem[0] == listItem[1]:
                    raise Exception(
                        "Invalid timescale declaration: you cannot have an interval in which the starting value and ending value are equal (such an interval should be declared as a point).")

            for listItemToCompare in ts:
                if listItem == listItemToCompare and listItem is not listItemToCompare:
                    raise Exception(
                        "Invalid timescale declaration: you cannot include the same point or interval more than once.")
                ...
                if listItem is not listItemToCompare:
                    if isinstance(listItem, list) and isinstance(listItemToCompare, list):
                        if (listItem[0] >= listItemToCompare[0] and listItem[0] <= listItemToCompare[1]) or (listItem[1] >= listItemToCompare[0] and listItem[1] <= listItemToCompare[1]):
                            raise Exception(
                                "Invalid timescale declaration: you cannot have overlapping intervals.")
                    ...
                    if isinstance(listItem, list) and not isinstance(listItemToCompare, list):
                        if listItemToCompare >= listItem[0] and listItemToCompare <= listItem[1]:
                            raise Exception(
                                "Invalid timescale declaration: you cannot declare a point that is included in an interval (you cannot declare a value more than once).")
    ...
    def dintegral(self, f, t, s, throwExceptions=True):
        ...
        if not tIsAnElement and not sIsAnElement:
            message = "The bounds of the dintegral function, t = " + \
                str(t) + " and s = " + str(s) + \
                ", are not elements of the timescale."
            errorOccurred = True

        elif not tIsAnElement:
            message = "The upper bound of dintegral function, t = " + \
                str(t) + ", is not an element of timescale."
            errorOccurred = True

        elif not sIsAnElement:
            message = "The lower bound of dintegral function, s = " + \
                str(s) + ", is not an element of timescale."
            errorOccurred = True
        ...
        sumOfIntegratedIntervals = sum(
            [self.integrate_complex(f, x[0], x[1]) for x in intervals])
    ...
    def compute_potentially_infinite_timescale(self, f, ts_generator_function, ts_generator_arguments):
        ...
        def wrapper_function(ts_gen_arg_mpf):
            ...
            self.validate_generated_timescale_value_pair(
                ts_item, next_ts_item, ts_generator_function)

            print("wrapper_function: ts_gen_arg =", ts_gen_arg)
            print("wrapper_function: ts_generator_arguments =",
                  ts_generator_arguments)
            ...
            if isinstance(ts_item, list):
                ...
                interval_result = self.integrate_complex(
                    f, ts_item[0], ts_item[1])
                ...
                if ts_generator_arguments[1] > ts_gen_arg:
                    ...
                    step_after_interval = (
                        next_ts_item - ts_item[1]) * f(ts_item[1])

                print("interval_result =", interval_result)
                print("step_after_interval =", step_after_interval)

                print("*****wrapper_function: RETURNING interval_result + step_after_interval =",
                      interval_result + step_after_interval)
                print()
                ...
                print(
                    "*****wrapper_function: RETURNING discrete result value =", discrete_result)
                print()
    ...
    def compute_potentially_infinite_timescale_for_t(self, f, t_target, t_0, ts_generator_function, ts_generator_arguments, signif_count=10, signif_threshold=0.0000001, increasing_always_signif=True):
        ...
        def wrapper_function(ts_gen_arg):
            ...
            if isinstance(ts_item, list):
                ...
                if ts_item[0] > t_target:
                    raise Exception("ts_item[0] = " + str(ts_item[0]) + " was greater than t_target = " + str(
                        t_target) + " -- the timescale does not contain t_target")
                ...
                if t_target > ts_item[1]:
                    ...
                    interval_result = self.integrate_complex(
                        f, ts_item[0], t_target)

                    print(
                        "*****wrapper_function: t_target <= ts_item[1] -> RETURNING interval_result =", interval_result)
                ...
                if ts_item > t_target:
                    raise Exception("ts_item = " + str(ts_item) + " was greater than t_target = " + str(
                        t_target) + " -- the timescale does not contain t_target")
        ...
        result = self.special_sum_function(f, t_target, t_0, wrapper_function, ts_generator_function, ts_generator_arguments,
                                           prior_results_significance_count=signif_count, significance_limit=signif_threshold, increasing_always_signif=increasing_always_signif)
    ...
    def special_sum_function(self, f, t_target, t_0, wrapper_function, ts_generator_function, ts_generator_arguments, prior_results_significance_count, significance_limit, increasing_always_signif):
        ...
        iterations_limit = ts_generator_arguments[1] - \
            ts_generator_arguments[0]
        found_t_target = False

        print("iterations_limit =", iterations_limit)
        print("prior_results_significance_count =",
              prior_results_significance_count)
        ...
        while iteration < iterations_limit:
            ...
            find_t_0_dictionary_result = self.special_sum_function_find_t_0(
                f, t_target, t_0, wrapper_function, ts_generator_function, ts_generator_arg, ts_generator_arguments,)
            ...
            if find_t_0_dictionary_result["found_t_0"] is True:
                ...
                prior_results[iteration %
                              prior_results_significance_count] = find_t_0_dictionary_result["result"]
        ...
        if found_t_target is False:
            while iteration < iterations_limit:
                ...
                prior_results[iteration %
                              prior_results_significance_count] = dictionary_result["result"]
                ...
                if iteration >= prior_results_significance_count:
                    print("special_sum_function: checking significance of the last " +
                          str(prior_results_significance_count) + " results:")
                    ...
                    if self.insignificant_prior_results(iteration, prior_results, prior_results_significance_count, significance_limit, increasing_always_signif) is True:
                        print(
                            "special_sum_function: ***WARNING***: insignificant results detected: returning result before t_target was found")
                        break
        ...
        if iteration == iterations_limit:
            print(
                "special_sum_function: iterations_limit reached -- t_target may not have been found")
        ...
        if isinstance(ts_item, list):
            if ts_item[0] <= t_0 <= ts_item[1]:
                if t_target > ts_item[1]:
                    ...
                    print("*****find_t_0: RETURNING interval_result + step_after_interval =",
                          interval_result + step_after_interval)
                    print()
                    ...
                    print(
                        "*****special_sum_function_find_t_0: t_target <= ts_item[1] -> RETURNING interval_result =", interval_result)

                    return {"result": interval_result, "ts_generator_arg": ts_generator_arg, "found_t_0": True, "found_t_target": True}

            elif ts_item[0] > t_0:
                raise Exception(
                    "ts_item[0] > t_0 --  t_0 was not in the timescale")
            ...
            if t_0 == ts_item:
                if t_0 != t_target:
                    ...
                    print(
                        "*****find_t_0: RETURNING discrete result value =", discrete_result)
                    print()
                ...
                raise Exception(
                    "ts_item > t_0 --  t_0 was not in the timescale")
    ...
    def insignificant_prior_results(self, iteration, prior_results, prior_results_significance_count, significance_limit, increasing_always_signif):
        ...
        last_obtained_result_abs_val = abs(
            prior_results[(iteration) % prior_results_significance_count])
        ...
        while i < prior_results_significance_count:
            ...
            print("abs(prior_results[" + str((iteration + i) % prior_results_significance_count) + "]) =", abs(
                prior_results[(iteration + i) % prior_results_significance_count]))

            obtained_result_abs_val = abs(
                prior_results[(iteration + i) % prior_results_significance_count])
    ...
    def validate_generated_timescale_value_pair(self, ts_item, next_ts_item, ts_generator_function):
        description = "The given function " + \
            str(locals().get("ts_generator_function")) + \
            " did not generate a strictly increasing timescale."

        if isinstance(ts_item, list):
            if isinstance(next_ts_item, list):
                if ts_item[1] >= next_ts_item[0]:
                    raise Exception("(ts_item[1] = " + str(ts_item[1]) + ") >= (next_ts_item[0] = " + str(
                        next_ts_item[0]) + ")\n" + description)

            else:
                if ts_item[1] >= next_ts_item:
                    raise Exception(
                        "(ts_item[1] = " + str(ts_item[1]) + ") >= (next_ts_item = " + str(next_ts_item) + ")\n" + description)

        else:
            if isinstance(next_ts_item, list):
                if ts_item >= next_ts_item[0]:
                    raise Exception("(ts_item = " + str(ts_item) + ") >= (next_ts_item[0] = " + str(
                        next_ts_item[0]) + ")\n" + description)

            else:
                if ts_item >= next_ts_item:
                    raise Exception(
                        "(ts_item = " + str(ts_item) + ") >= (next_ts_item = " + str(next_ts_item) + ")\n" + description)
    ...
    def integrate_complex(self, f, s, t, **kwargs):
        ...
        real_result = float(mpmath.nstr(mpmath.quad(
            real_component, [s, t], **kwargs), n=15))
        imaginary_result = float(mpmath.nstr(mpmath.quad(
            imaginary_component, [s, t], **kwargs), n=15))
    ...
    def solve_ode_for_t(self, y_0, t_0, t_target, y_prime):  # Note: y(t_0) = y_0
        ...
        if t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_ode_for_t: t_0 is not a value in the timescale.")

        if not t_in_ts and t_0_in_ts:
            raise Exception(
                "solve_ode_for_t: t_target is not a value in the timescale.")

        if not t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_ode_for_t: t_0 and t_target are not values in the timescale.")
        ...
        if t_0 == t_target:
            ...
            raise Exception(
                "solve_ode_for_t: t_0 cannot be greater than t_target.")
        ...
        # Technically safer than "while True:"
        while self.isInTimescale(t_current):
            if discretePoint:
                ...
                y_sigma_of_t_current = y_current + \
                    y_prime(t_current, y_current) * self.mu(t_current)
                ...
                if self.isDiscretePoint(t_current):
                    raise Exception(
                        "t_current is NOT in a list/interval! Something went wrong!")

                else:
                    interval_of_t_current = self.getCorrespondingInterval(
                        t_current)
                    ...
                    if t_target <= interval_of_t_current[1] and t_target >= interval_of_t_current[0]:
                        ...
                        # print()
                        ODE_integration_result = ODE.integrate(
                            interval_of_t_current[1])
    ...
    def solve_ode_for_t_with_odeint(self, y_0, t_0, t_target, y_prime, stepSize=0.0001):
        ...
        if t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_ode_for_t_with_odeint: t_0 is not a value in the timescale.")

        if not t_in_ts and t_0_in_ts:
            raise Exception(
                "solve_ode_for_t_with_odeint: t_target is not a value in the timescale.")

        if not t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_ode_for_t_with_odeint: t_0 and t_target are not values in the timescale.")
        ...
        if t_0 == t_target:
            ...
            raise Exception(
                "solve_ode_for_t_with_odeint: t_0 cannot be greater than t_target.")
        ...
        while self.isInTimescale(t_current):
            if discretePoint:
                ...
                y_sigma_of_t_current = y_current + \
                    y_prime(y_current, t_current) * self.mu(t_current)
                ...
                if self.isDiscretePoint(t_current):
                    ...
                    if t_target <= interval_of_t_current[1] and t_target >= interval_of_t_current[0]:
                        ...
                        current_interval = np.arange(
                            t_current, t_target + stepSize, stepSize)

                        # print(current_interval)
                        # print()

                        ODE_integration_result = integrate.odeint(
                            y_prime, y_current, current_interval)
                        ODE_integration_result = ODE_integration_result[len(
                            ODE_integration_result) - 1]
                        ...
                        current_interval = np.arange(
                            t_current, interval_of_t_current[1] + stepSize, stepSize)
    ...
    def solve_ode_system_for_t(self, y_0, t_0, t_target, y_prime, stepSize=0.0001):
        ...
        if t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_ode_system_for_t: t_0 is not a value in the timescale.")

        if not t_in_ts and t_0_in_ts:
            raise Exception(
                "solve_ode_system_for_t: t_target is not a value in the timescale.")

        if not t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_ode_system_for_t: t_0 and t_target are not values in the timescale.")
        ...
        if t_0 == t_target:
            ...
            raise Exception(
                "solve_ode_system_for_t: t_0 cannot be greater than t_target.")
        ...
        while self.isInTimescale(t_current):
            if discretePoint:
                ...
                temp1 = list(map(lambda x: x * self.mu(t_current),
                                 y_prime(y_current, t_current)))
    ...
    def solve_dde_for_t(self, y_values, t_0, t_target, y_prime, JiTCDDE=None, stepSize=0.01, return_all_results=False):
        ...
        for y_value_key in y_values:
            if not self.isInTimescale(y_value_key):
                raise Exception("The initial y value, t =",
                                y_value_key, " is not in the timescale")
        ...
        if t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_dde_for_t: t_0 is not a value in the timescale.")

        if not t_in_ts and t_0_in_ts:
            raise Exception(
                "solve_dde_for_t: t_target is not a value in the timescale.")

        if not t_in_ts and not t_0_in_ts:
            raise Exception(
                "solve_dde_for_t: t_0 and t_target are not values in the timescale.")
        ...
        if t_0 == t_target:
            ...
            raise Exception(
                "solve_dde_for_t: t_0 cannot be greater than t_target.")
        ...
        while self.isInTimescale(t_current):
            if discretePoint:
                ...
                print("t_target =", t_target)
                print("y_prime(t_current, y_values) =",
                      y_prime(t_current, y_values))
                print("self.mu(t_current) =", self.mu(t_current))
                print()

                y_sigma_of_t_current = y_values[t_current] + \
                    y_prime(t_current, y_values) * self.mu(t_current)
                ...
                past_point = [t_current, [y_values[t_current]],
                              [y_sigma_of_t_current]]
                ...
                if self.isDiscretePoint(t_current):
                    ...
                    if t_target <= interval_of_t_current[1] and t_target >= interval_of_t_current[0]:
                        ...
                        for time in current_interval:
                            if time <= t_target:
                                DDE_integration_result = JiTCDDE.integrate_blindly(
                                    time)
                                all_results.append(DDE_integration_result[0])
                                print(
                                    "time =", time, " |  integration_result =", DDE_integration_result)

                        #---Testing-Code-Start---#

                        # The following should hold barring accuracy limitations: t_target != JiTCDDE.t
                        t_current = t_target
                        ...
                        print(
                            "y_current = y_values[t_current] =", y_values[t_current])
                        print()
                        ...
                        print("time =", t_current,
                              "| DDE_integration_result =", DDE_integration_result)
                        print()
                        ...
                        # The following should hold barring accuracy limitations: interval_of_t_current[1] == JiTCDDE.t
                        t_current = interval_of_t_current[1]
                        y_values[t_current] = DDE_integration_result[0]
    ...
    def delay(self, delay_value, error=None):
        if error is None:
            if not self.isInTimescale(delay_value):
                raise Exception("The delay value =", delay_value,
                                " was not in the timescale")

        else:
            if not self.isInTimescaleWithError(delay_value, error):
                raise Exception("The delay value =", delay_value,
                                " was not in the timescale with error =", error)
    ...
    def initializeJiTCDDE(self, y_prime_jitcdde, past_function, arg_max_delay, arg_times_of_interest, c_backend):
        ...
        DDE.past_from_function(
            past_function, times_of_interest=arg_times_of_interest)
    ...
    def isDiscretePoint(self, t):
        ...
        raise Exception(
            "isDiscretePoint(): t was neither a discrete point nor in an interval!")
    ...
    def plot(self, f, stepSize=0.01, discreteStyle='b.', intervalStyle='r-', **kwargs):
        ...
        for xyIntervalPointsPair in intervals:
            ...
            plt.plot(
                xyIntervalPointsPair[0], xyIntervalPointsPair[1], intervalStyle, **kwargs)
    ...
    def scatter(self, f, stepSize=0.01, **kwargs):
        ...
        for xyIntervalPointsPair in intervals:
            plt.scatter(xyIntervalPointsPair[0],
                        xyIntervalPointsPair[1], **kwargs)
